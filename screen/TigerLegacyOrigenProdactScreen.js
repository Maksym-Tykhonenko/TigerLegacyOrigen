...
const TigerLegacyOrigenProdactScreen = ({navigation, route}) => {
  ...
  const setData = async () => {
    try {
      ...
      await AsyncStorage.setItem('TigerLegacyOrigenProdactScreen', jsonData);
  ...
  const getData = async () => {
    try {
      const jsonData = await AsyncStorage.getItem(
        'TigerLegacyOrigenProdactScreen',
      );
      if (jsonData !== null) {
...
export default TigerLegacyOrigenProdactScreen;
